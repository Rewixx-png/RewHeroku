...
class Web(root.Web):
    def __init__(self, **kwargs):
        ...
        # <<< НАЧАЛО ИЗМЕНЕНИЙ >>>
        self._pin_sessions = {} # Хранилище для временных PIN-кодов
        # <<< КОНЕЦ ИЗМЕНЕНИЙ >>>
    ...
    async def request_pin(self, request: web.Request) -> web.Response:
        """Handles PIN request from the frontend"""
        ...
        for sid, data in list(self._pin_sessions.items()):
            if time.time() - data["timestamp"] > 300: # 5 minute expiration
                del self._pin_sessions[sid]

        try:
            # Use the first available client to send the PIN
    ...
    async def verify_pin(self, request: web.Request) -> web.Response:
        """Verifies the PIN sent from the frontend"""
        try:
        ...
        # PIN is correct, create a persistent session
        ...
        response.set_cookie("session", session_cookie, max_age=3600) # 1 hour cookie
        
        return response
    
    # Заменяем старую web_auth на новую логику
    async def web_auth(self, request: web.Request) -> web.Response:
        ...
        # Если сессии нет, мы больше не используем старый метод.
        # Теперь JS должен будет вызвать /request_pin.
        # Просто возвращаем ошибку, если кто-то пытается вызвать старый эндпоинт.
